	...
	ShadeCol &origLight, float probLight,int flags)
{
    ...
    cs = cs < 0.0f ? 0.0f : cs;
	float ret_amount = 0.f;

    ShadeCol ret(0.f);
    if(flags & FBRDF_DIFFUSE){
		float probReflection=cs*2.f;
		float weight = 1.f; //NOTE(Vidar): Multiple importance sampling factor
		// Apply combined sampling ONLY if GI is on which will pick up the rest
		// of the result
		if (rc.rayparams.localRayType & RT_IS_GATHERING_POINT) {
			weight = getReflectionWeight(probLight, probReflection);
			ret += m_diffuse_color*weight;
		}
    }
...
#undef DYNAMIC_FUNC_ARG_NAMES
        ...
        ret += reflect_color*weight;
		//ret_amount += weight / probReflection;
		/// result=shadowedLight*(0.5f*invProbReflection*weight)*cs;
		/// where invProbReflection is the inverse of the probability for the direction to be generated by the reflection sampler (i.e. the inverse of the differential solid angle
		/// covered by the direction) and
		/// @code
		/// weight=sqr(invProbLight)/(sqr(invProbReflection)+sqr(invProbLight));
		/// cs=direction*rc.rayresult.normal;
    }

    ret = shadowedLight*ret*0.5f*cs;
