...
// NOTE: When adding new parameters, check:
// http://docs.autodesk.com/3DSMAX/16/ENU/3ds-Max-SDK-Programmer-Guide/index.html?url=files/GUID-F35959BB-2660-492F-B082-56304C70293A.htm,topicNumber=d30e52807
                    ...
                    case IDC_WIFFILE_BUTTON:
                    {
                        switch(HIWORD(wParam)){
                            case BN_CLICKED:
                            {
                                ...
                                if(filename[0]!=0){
                                    const char *error=0;
									tlWeaveParameters* param = tl_weave_pattern_from_file(filename, &error);
									// TODO: Dynamic Load
					...
					case IDC_PATTERNEDITOR_BUTTON:
					{
						switch(HIWORD(wParam)){
							case BN_CLICKED:
							{
								...
								tlWeaveParameters* param = tl_pattern_editor(sm->m_weave_parameters);
								// TODO: Dynamic Load
...
void ThunderLoomMtl::Reset() {
	...
	tlWeaveParameters* param = tl_weave_pattern_from_data(warp_above, yarn_type, num_yarn_types, yarn_colors, pattern_width, pattern_height);
	// TODO: Dynamic Load
            ...
            case YARN_TYPE_CHUNK:
			{
                ...
                if(version==90){
                    ...
                    *(int*)data=1;
					...
					tlWeaveParameters* param = tl_weave_pattern_from_ptn(data, len, &error_buffer);
					// TODO: Dynamic Load
...
\*===========================================================================*/
// TODO: Make this use the common vray class.
...
void ThunderLoomMtl::deleteBSDF(const VR::VRayContext &rc, VR::BSDFSampler *b) {
	if (!b) return;
	BRDFThunderLoomSampler *bsdf=static_cast<BRDFThunderLoomSampler*>(b);
